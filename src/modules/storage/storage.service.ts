...
export class StorageService {
  ...
  async uploadFileForUser(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const normalizedImg = fileBase64.replace(
          /^data:image\/[a-z]+;base64,/,
          '',
        );
        const imageBuffer = Buffer.from(normalizedImg, 'base64');

        const bucket = firebaseAdmin.storage(firebaseAdmin.app()).bucket();
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (path.indexOf('http') >= 0) {
          resolve(
            `${path}${timestamp ? '&decache=' + timestamp.getTime() : ''}`,
          );
        } else {
          const bucket = firebaseAdmin
            .storage(firebaseAdmin.app())
          ...
          if (exists[0]) {
            ...
            resolve(
              `${url[0]}${timestamp ? '&decache=' + timestamp.getTime() : ''}`,
            );
          } else {
            resolve(null);
          }
