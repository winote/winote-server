...
export class UserService {
  ...
  protected async saveProfilePic(user: UpdateUserDto) {
    ...
    const firebasePath = await this.storageService.uploadFileForUser(
      filePath,
      'image/png',
      user.avatar,
    );
  ...
  private async updateFirebaseUser(
    ...
    await FirebaseProvider.authAdmin(async (auth: Auth) => {
      if (user.firebaseUid) {
        const createRequest = this.createFirebaseUserCreateRequest(
          user,
          passwordChanged,
        );
        ...
        this.LOGGER.warn(
          `User ${user.id} has been updated and don't have a Firebase registration!`,
        );
      }
    });
  }
  ...
  private createFirebaseUserCreateRequest(
    entity: User,
