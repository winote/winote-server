...
export class FirebaseProvider {
    ...
    public static async authAdmin(handler: ((auth: FirebaseAdmin.auth.Auth) => Promise<void>)): Promise<boolean> {
    ...
    public static async authClient(handler: ((auth: FirebaseAuthClient.Auth) => Promise<void>)): Promise<boolean> {
        await this.instance.init();
...
export class FirebaseUserNotFound extends UnauthorizedException {
    ...
    constructor(private readonly email) {super()}
