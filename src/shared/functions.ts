...
export function pad(text: string, width: number, caractere?: string) {
    ...
    text = text + '';
    return text.length >= width ? text : new Array(width - text.length + 1).join(caractere) + text;
}

export function setValueByPath(obj: any, path: string, value: any, separator: string) {
    let ref = obj;
...
export function subArray(input: number[], start: number, size: number): number[] {
    let result: number[] = [];
...
export function ValidationArrayNotEmptyException(field: string): BadRequestException {
    let message = {};
...
export function getUniqueObjectsArray(arrayValues: any, attribute: string) {
    return Array.isArray(arrayValues) ? arrayValues.reduce((acc, cur) => [...acc.filter((obj) => obj[attribute] !== cur[attribute]), cur], []) : arrayValues;
}
